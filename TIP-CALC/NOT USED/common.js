...
function isInteger(val) {
    if (isNumber(val)) { return val % 1 === 0; }
    else { return false; }
}
...
function clearForm(formObj) {
    var allElements = formObj.elements; for (i = 0; i < allElements.length; i++) {
        if (allElements[i].type == 'text') allElements[i].value = '';
    }
}

function formatNum(inNum) {
    outStr = "" + inNum; inNum = parseFloat(outStr);
    if ((outStr.length) > 10) { outStr = "" + inNum.toPrecision(10); }
    if (outStr.indexOf(".") > -1) {
...
function formatNum2(inNum, nlen, ncutoff) {
    inNum = parseFloat(inNum); inFloor = Math.floor(inNum);
...
function showquickmsg(inStr, isError) {
    if (isError) { inStr = "<font color=red>" + inStr + "</font>"; } gObj("coutput").innerHTML = inStr;
}
...
function showTopNav(navSection) {
    ...
    if (navSection == 1) 
    outPut += ' style="background-color: #518428;"';
    outPut += ' onClick="window.location=\'/financial-calculator.html\';">Financial</span></span>';
    outPut += '<span class="dropdown"><span class="dropbtn"';
    if (navSection == 2) 
    outPut += ' style="background-color: #518428;"';
    outPut += ' onClick="window.location=\'/fitness-and-health-calculator.html\';">Fitness &amp; Health</span></span>';
    outPut += '<span class="dropdown"><span class="dropbtn"';
    if (navSection == 3) 
    outPut += ' style="background-color: #518428;"';
    outPut += ' onClick="window.location=\'/math-calculator.html\';">Math</span></span>';
    outPut += '<span class="dropdown"><span class="dropbtn"';
    if (navSection == 4)
    outPut += ' style="background-color: #518428;"';
    outPut += ' onClick="window.location=\'/other-calculator.html\';">Other</span></span>';
...
var tooltip = function () {
    ...
    return {
        show: function (v, ism) {
            if (ism == 'm') { ismobile = true; }
            else { ismobile = false; }
            if (tt == null) {
                tt = document.createElement('div');
                tt.setAttribute('id', id);
                document.body.appendChild(tt); tt.style.opacity = 0;
            ...
            if (isvisible == 0) {
                ...
                if (document.all) {
                    ...
                    y = (document.documentElement && document.documentElement.scrollTop) ? document.documentElement.scrollTop : document.body.scrollTop; x += window.event.clientX; y += window.event.clientY;
                }
                else { x = e.pageX; y = e.pageY; }
                if (ismobile) { tt.style.left = "0px"; }
                else { tt.style.left = (x + 5) + "px"; }
            ...
            if ((a != endalpha && d == 1) || (a != 0 && d == -1)) {
                ...
                else if (alpha < speed && d == -1) { i = a; } alpha = a + (i * d); tt.style.opacity = alpha * .01; tt.style.filter = 'alpha(opacity=' + alpha + ')';
            }
            else {
                clearInterval(tt.timer);
                if (d == -1) { tt.style.display = 'none' }
            }
        }, hide: function () { clearInterval(tt.timer); isvisible = 0; tt.style.display = 'none'; }
    };
}();
